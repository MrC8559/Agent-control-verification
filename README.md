...
The **v0.1.0 first real-host evidence milestone is complete** for the tested Windows Codex CLI `0.154.0` environment and controlled `apply_patch` scenarios. [ACV v0.1.0](https://github.com/MrC8559/Agent-control-verification/releases/tag/v0.1.0) is formally published and remains experimental, pre-alpha research.
...
See the [consolidated evidence report](docs/reports/2026-09-19-codex-0.154.0.md) and its validated redacted bundles. The live experiment used package `0.0.3` at revision `04efa04f05d95efa30963e8a4190556e60569127`. The released package version is `0.1.0`; the live experiment was not rerun for this version change. ACV output is not a certification or security guarantee.
...
Further research is deferred pending a separate prioritization decision. No later research has started. The report lists bounded open questions; an Observatory index and additional execution paths remain separately scoped future work.
